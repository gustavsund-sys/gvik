...
async function boot(){
  ...
  const pixelRatio=Math.max(1,window.devicePixelRatio||1),renderCap=matchMedia('(pointer:coarse)').matches?2:1.5;viewer.resolutionScale=Math.min(pixelRatio,renderCap)/pixelRatio;viewer.scene.postProcessStages.fxaa.enabled=true;viewer.scene.globe.maximumScreenSpaceError=1.5;viewer.scene.globe.baseColor=C.Color.fromCssColorString('#254739'); viewer.scene.skyAtmosphere.show=false; viewer.scene.globe.enableLighting=false;const cameraControls=viewer.scene.screenSpaceCameraController;cameraControls.minimumZoomDistance=70;cameraControls.rotateEventTypes=[C.CameraEventType.RIGHT_DRAG];cameraControls.translateEventTypes=[C.CameraEventType.LEFT_DRAG];cameraControls.zoomEventTypes=[C.CameraEventType.WHEEL,C.CameraEventType.PINCH];cameraControls.tiltEventTypes=[C.CameraEventType.MIDDLE_DRAG,C.CameraEventType.PINCH];viewer.camera.moveEnd.addEventListener(layoutMapLabels);if(matchMedia('(pointer:coarse)').matches)$('hint').textContent='Dra: flytta · Nyp: zooma och luta';reset(); await loadOrthophoto();
