...
function handleMapClick(click){
  ...
  if(!state.mode){if(document.body.classList.contains('user-mode')){const p=mapPoint(click.position),tees=state.areas.filter(a=>a.type==='tee'&&String(a.hole)===state.view),tee=p&&tees.find(a=>pointInPolygon(p,a.points))||p&&tees.reduce((nearest,a)=>{const metres=distanceBetween(p,areaCenter(a.points));return metres<=24&&(!nearest||metres<nearest.metres)?{area:a,metres}:nearest},null)?.area;if(tee)selectPlayerTee(tee.id);return}const picked=viewer.scene.drillPick(click.position).find(hit=>entityProperty(hit?.id,'areaId')),areaId=entityProperty(picked?.id,'areaId');if(areaId)selectArea(areaId);return} const p=mapPoint(click.position); if(!p){status('Markera inom Gustavsviksbanans område.');return}
