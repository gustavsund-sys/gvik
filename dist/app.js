...
function handleMapClick(click){
  ...
  if(!state.mode){const picked=viewer.scene.drillPick(click.position).find(hit=>entityProperty(hit?.id,'areaId')),areaId=entityProperty(picked?.id,'areaId');if(areaId){if(document.body.classList.contains('user-mode'))selectPlayerTee(areaId);else selectArea(areaId)}return} const p=mapPoint(click.position); if(!p){status('Markera inom Gustavsviksbanans område.');return}
