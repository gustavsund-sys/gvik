...
const state = {view:'overview', playing:false, progress:0, ready:false, mode:null, points:[], routes:{}, boundary:[], guides:{}, areas:[], axes:{}, defaultAxes:{}, axisTees:{}, par3:{}, holeViews:{}, startView:null, terrain:false, localTerrain:false, imagery:false};
...
function layoutTeeHitTargets(){if(!viewer)return;const layer=$('tee-hit-layer');for(const button of layer.children){const area=state.areas.find(a=>a.id===button.dataset.areaId),points=area?.points.map(p=>viewer.scene.cartesianToCanvasCoordinates(terrainPosition(p,1),new C.Cartesian2())).filter(Boolean)||[];if(points.length<3){button.hidden=true;continue}const xs=points.map(p=>p.x),ys=points.map(p=>p.y),minX=Math.min(...xs),maxX=Math.max(...xs),minY=Math.min(...ys),maxY=Math.max(...ys),x=(minX+maxX)/2,y=(minY+maxY)/2;button.hidden=x<0||y<0||x>layer.clientWidth||y>layer.clientHeight;button.style.left=`${x}px`;button.style.top=`${y}px`;button.style.width=`${Math.max(56,maxX-minX+28)}px`;button.style.height=`${Math.max(56,maxY-minY+28)}px`}}
function renderTeeHitTargets(){const layer=$('tee-hit-layer');if(!layer)return;layer.replaceChildren();if(!viewer||!document.body.classList.contains('user-mode')||state.view==='overview'||state.mode)return;for(const area of state.areas.filter(a=>a.type==='tee'&&String(a.hole)===state.view&&a.id!==state.axisTees[state.view])){const button=document.createElement('button');button.type='button';button.className='tee-hit-target';button.dataset.areaId=area.id;button.setAttribute('aria-label',`Välj ${area.name||`tee för hål ${state.view}`}`);const activate=event=>{event.preventDefault();event.stopPropagation();selectPlayerTee(area.id)};button.onpointerup=activate;button.onclick=activate;layer.append(button)}layoutTeeHitTargets()}
function layoutMapLabels(){if(!viewer)return;layoutTeeHitTargets();if(!mapLabelLayout.length)return;const placed=[];for(const item of [...mapLabelLayout].sort((a,b)=>a.priority-b.priority)){const position=item.entity.position?.getValue?.(C.JulianDate.now())||item.entity.position,screen=position&&C.SceneTransforms.worldToWindowCoordinates(viewer.scene,position);if(!screen)continue;let chosen=0;for(const dx of [0,item.width*.62,-item.width*.62,item.width*1.24,-item.width*1.24,item.width*1.86,-item.width*1.86]){const box={left:screen.x+dx-item.width/2,right:screen.x+dx+item.width/2,top:screen.y+item.baseY-item.height,bottom:screen.y+item.baseY};if(!placed.some(other=>labelsOverlap(box,other))){chosen=dx;placed.push(box);break}}const offset=new C.Cartesian2(chosen,item.baseY);if(item.entity.billboard)item.entity.billboard.pixelOffset=offset;else if(item.entity.label)item.entity.label.pixelOffset=offset}viewer.scene.requestRender()}
function drawAreasAndAxis(){
  clearEntities(areaEntities);clearEntities(axisEntities);mapLabelLayout=[];$('tee-hit-layer')?.replaceChildren();if(axisDragEntity){viewer.entities.remove(axisDragEntity);axisDragEntity=null}if(state.view==='overview')return;
...
function selectPlayerTee(id){const area=state.areas.find(a=>a.id===id&&area.type==='tee'&&String(a.hole)===state.view),axis=state.axes[state.view];if(!area)return;state.axisTees[state.view]=area.id;if(axis)axis[0]=areaCenter(area.points);selectedAreaId=null;persist();drawAreasAndAxis();updateGuideUi();status(`${area.name||'Teeområdet'} är valt. Alla längder mäts nu från denna tee.`)}
function entityProperty(entity,key){const value=entity?.[key];return value?.getValue?value.getValue(C.JulianDate.now()):value}
function teeAtScreenPosition(position){const tees=state.areas.filter(a=>a.type==='tee'&&String(a.hole)===state.view);let nearest=null;for(const tee of tees){const screenPoints=tee.points.map(p=>C.SceneTransforms.worldToWindowCoordinates(viewer.scene,terrainPosition(p,1))).filter(Boolean);if(screenPoints.length<3)continue;const polygon=screenPoints.map(p=>[p.x,p.y]);if(pointInPolygon([position.x,position.y],polygon))return tee;const center=polygon.reduce((sum,p)=>[sum[0]+p[0]/polygon.length,sum[1]+p[1]/polygon.length],[0,0]),pixels=Math.hypot(position.x-center[0],position.y-center[1]);if(pixels<=44&&(!nearest||pixels<nearest.pixels))nearest={tee,pixels}}return nearest?.tee||null}
function selectTeeAtScreenPosition(position){if(!document.body.classList.contains('user-mode')||state.view==='overview'||state.mode)return false;const tee=teeAtScreenPosition(position);if(!tee)return false;selectPlayerTee(tee.id);return true}
function handleMapClick(click){
  if(suppressMapClick){suppressMapClick=false;return}
  if(!state.mode){if(document.body.classList.contains('user-mode')){selectTeeAtScreenPosition(click.position);return}const picked=viewer.scene.drillPick(click.position).find(hit=>entityProperty(hit?.id,'areaId')),areaId=entityProperty(picked?.id,'areaId');if(areaId)selectArea(areaId);return} const p=mapPoint(click.position); if(!p){status('Markera inom Gustavsviksbanans område.');return}
