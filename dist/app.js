...
const state = {view:'overview', playing:false, progress:0, ready:false, mode:null, points:[], routes:{}, boundary:[], guides:{}, areas:[], axes:{}, defaultAxes:{}, axisTees:{}, terrain:false, localTerrain:false, imagery:false};
...
try {
  ...
  if (saved.defaultAxes && typeof saved.defaultAxes === 'object') for(const [id,axis] of Object.entries(saved.defaultAxes))if(Array.isArray(axis)&&axis.length===4&&axis.every(validPoint))state.defaultAxes[id]=axis;
...
async function boot(){
  ...
  viewer.screenSpaceEventHandler.setInputAction(event=>{const picked=viewer.scene.pick(event.position),handle=picked?.id?.axisHandle;if(!handle)return;if(handle.index===0){status('Teepunkten är låst. Välj tee-box i menyn för att flytta den.');return}draggingAxisPoint={...handle};viewer.scene.screenSpaceCameraController.enableInputs=false;setAxisDragStyle(true);status(`Dra axelpunkt ${handle.index+1} till rätt plats.`);},C.ScreenSpaceEventType.LEFT_DOWN);
  viewer.screenSpaceEventHandler.setInputAction(event=>{if(!draggingAxisPoint)return;const p=mapPoint(event.endPosition);if(!p)return;state.axes[draggingAxisPoint.hole][draggingAxisPoint.index]=p;updateAxisWhileDragging();},C.ScreenSpaceEventType.MOUSE_MOVE);
  viewer.screenSpaceEventHandler.setInputAction(()=>{if(!draggingAxisPoint)return;const drag=draggingAxisPoint,axis=state.axes[drag.hole];if(drag.index===0){const snap=nearestTeeSnap(axis[0],drag.hole);if(snap){axis[0]=snap;status(`Teeänden snappade till markerat teeområde för hål ${drag.hole}.`)}else status(`Axelpunkt 1 flyttades. Markera ett teeområde för automatisk snapning.`)}else status(`Axelpunkt ${drag.index+1} flyttades.`);persist();draggingAxisPoint=null;viewer.scene.screenSpaceCameraController.enableInputs=true;drawAreasAndAxis();updateGuideUi();},C.ScreenSpaceEventType.LEFT_UP);
}
...
$('create-axis').onclick=()=>{state.axes[state.view]=defaultAxis();delete state.defaultAxes[state.view];updateGuideUi();lockAxisToSelectedTee(state.view);persist();drawAreasAndAxis();redrawGuides();updateGuideUi();status(`Fyra axelpunkter skapades för hål ${state.view}. Teepunkten är låst till vald tee-box.`);};
...
$('save-axis').onclick=()=>{const axis=state.axes[state.view];if(!axis)return;state.defaultAxes[state.view]=axis.map(p=>p.slice());persist();updateGuideUi();status(`Standardaxeln för hål ${state.view} sparades.`);};
