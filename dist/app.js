...
const state = {view:'overview', playing:false, progress:0, ready:false, mode:null, points:[], routes:{}, boundary:[], guides:{}, areas:[], axes:{}, defaultAxes:{}, axisTees:{}, par3:{}, holeViews:{}, startView:null, desktopStartView:null, objects3d:[], vegetationSections:[], defaultsRevision:0, terrain:false, localTerrain:false, imagery:false};
...
let C, viewer, localTerrain, outlineSource, orthophotoLayer, orthophotoBounds, courseMaskEntity, objectBuilder;
...
function drawAreasAndAxis(){
  clearEntities(areaEntities);clearEntities(axisEntities);mapLabelLayout=[];if(axisDragEntity){viewer.entities.remove(axisDragEntity);axisDragEntity=null}if(state.view==='overview'){drawGpsAxis();return}
  for(const area of state.areas.filter(a=>String(a.hole)===state.view)){
    ...
    const polygon=viewer.entities.add({polygon:{hierarchy:C.Cartesian3.fromDegreesArray(renderPoints.flat()),material:C.Color.fromCssColorString(color).withAlpha(selected||activeTee ? .5 : .34),outline:true,outlineColor:C.Color.fromCssColorString(color).withAlpha(activeTee?1:.72),heightReference:C.HeightReference.CLAMP_TO_GROUND,classificationType:C.ClassificationType.TERRAIN,zIndex:selected?12:8}});polygon.addProperty('areaId');polygon.areaId=area.id;areaEntities.push(polygon);if(feathered){const feather=[...renderPoints,renderPoints[0]];areaEntities.push(viewer.entities.add({polyline:{positions:C.Cartesian3.fromDegreesArray(feather.flat()),width:activeTee?12:9,material:C.Color.fromCssColorString(color).withAlpha(activeTee?.3:.16),clampToGround:true,zIndex:selected?11:7}}))}
    const showAreaLabel=!document.body.classList.contains('user-mode')||area.type!=='tee'||activeTee;
  ...
  const axis=state.axes[state.view];if(axis){axisEntities.push(viewer.entities.add({polyline:{positions:C.Cartesian3.fromDegreesArray(axis.flat()),width:4,material:new C.PolylineDashMaterialProperty({color:C.Color.fromCssColorString('#ffffff'),dashLength:18}),clampToGround:true}}));
  axis.forEach((p,index)=>{const direct=index?new C.EllipsoidGeodesic(C.Cartographic.fromDegrees(...axis[0]),C.Cartographic.fromDegrees(...p)).surfaceDistance:0,text=index===0?'TEE':index===axis.length-1?`${Math.round(direct)} m · GREEN`:`${Math.round(direct)} m`,entity=viewer.entities.add({position:terrainPosition(p),point:{pixelSize:index===0?22:18,color:C.Color.fromCssColorString(index===0?'#f4d35e':'#ffffff').withAlpha(.5),outlineColor:C.Color.fromCssColorString('#0b2922').withAlpha(.5),outlineWidth:4,heightReference:C.HeightReference.NONE,disableDepthTestDistance:Number.POSITIVE_INFINITY},label:{text,font:'600 13px Inter, system-ui, sans-serif',fillColor:C.Color.fromCssColorString('#08251f'),showBackground:true,backgroundColor:C.Color.fromCssColorString(index===0?'#f4d35e':'#ffffff').withAlpha(.5),pixelOffset:new C.Cartesian2(0,-18),verticalOrigin:C.VerticalOrigin.BOTTOM,heightReference:C.HeightReference.NONE,disableDepthTestDistance:Number.POSITIVE_INFINITY}});entity.addProperty('axisHandle');entity.axisHandle={hole:state.view,index};axisEntities.push(entity);mapLabelLayout.push({entity,width:Math.max(54,text.length*7.5+18),height:28,baseY:-18,priority:2,kind:'axis'})})}drawGpsAxis();requestAnimationFrame(layoutMapLabels);viewer.scene.requestRender();
}
...
async function boot(){
  ...
  await loadEditorDefaults(); viewer=new C.Viewer('map',{baseLayer:false,baseLayerPicker:false,geocoder:false,homeButton:false,sceneModePicker:false,navigationHelpButton:false,timeline:false,animation:false,fullscreenButton:false,infoBox:false,selectionIndicator:false,creditContainer:$('map-credits'),requestRenderMode:true,maximumRenderTimeChange:Infinity,useBrowserRecommendedResolution:false});
  const touchNavigation=isTouchDevice(),pixelRatio=Math.max(1,window.devicePixelRatio||1),renderCap=touchNavigation?2:1.5;viewer.resolutionScale=Math.min(pixelRatio,renderCap)/pixelRatio;viewer.scene.postProcessStages.fxaa.enabled=true;viewer.scene.globe.maximumScreenSpaceError=1.5;viewer.scene.globe.baseColor=C.Color.fromCssColorString('#254739'); viewer.scene.skyAtmosphere.show=false; viewer.scene.globe.enableLighting=false;const cameraControls=viewer.scene.screenSpaceCameraController;cameraControls.minimumZoomDistance=70;cameraControls.rotateEventTypes=touchNavigation?[C.CameraEventType.LEFT_DRAG]:[C.CameraEventType.RIGHT_DRAG];cameraControls.translateEventTypes=touchNavigation?[]:[C.CameraEventType.LEFT_DRAG];cameraControls.zoomEventTypes=[C.CameraEventType.WHEEL,C.CameraEventType.PINCH];cameraControls.tiltEventTypes=[C.CameraEventType.MIDDLE_DRAG,C.CameraEventType.PINCH];viewer.camera.moveEnd.addEventListener(layoutMapLabels);if(touchNavigation)$('hint').textContent='Dra: flytta · Nyp: zooma och luta';reset(); await loadOrthophoto();
