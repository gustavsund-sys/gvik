...
function drawAreasAndAxis(){
  ...
  for(const area of state.areas.filter(a=>String(a.hole)===state.view)){
    const [color,label]=areaVisualStyle(area),labelPlacement=areaLabelPosition(area),displayName=(area.name||`${label} (Hål ${area.hole})`).trim(),selected=area.id===selectedAreaId,soft=area.type==='tee'||area.type==='green',renderPoints=soft?smoothClosedPolygon(area.points):area.points;
    const polygon=viewer.entities.add({polygon:{hierarchy:C.Cartesian3.fromDegreesArray(renderPoints.flat()),material:C.Color.fromCssColorString(color).withAlpha(selected ? .5 : .34),outline:true,outlineColor:C.Color.fromCssColorString(color).withAlpha(.72),heightReference:C.HeightReference.CLAMP_TO_GROUND,classificationType:C.ClassificationType.TERRAIN,zIndex:selected?12:8}});polygon.addProperty('areaId');polygon.areaId=area.id;areaEntities.push(polygon);if(soft){const feather=[...renderPoints,renderPoints[0]];areaEntities.push(viewer.entities.add({polyline:{positions:C.Cartesian3.fromDegreesArray(feather.flat()),width:9,material:C.Color.fromCssColorString(color).withAlpha(.16),clampToGround:true,zIndex:selected?11:7}}))}
...
$('settings-toggle').onclick=()=>{if(document.body.classList.contains('builder-mode')){applyUiMode('user');status('Spelarläget visas.');return}const password=prompt('Lösenord till kartbyggaren:');if(password==='gvik2026'){applyUiMode('builder');status('Kartbyggaren är upplåst.')}else if(password!==null)alert('Fel lösenord.');};
