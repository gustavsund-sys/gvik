...
function drawAreasAndAxis(){
  ...
  axis.forEach((p,index)=>{const direct=index?new C.EllipsoidGeodesic(C.Cartographic.fromDegrees(...axis[0]),C.Cartographic.fromDegrees(...p)).surfaceDistance:0;const entity=viewer.entities.add({position:C.Cartesian3.fromDegrees(...p),point:{pixelSize:index===0?22:18,color:C.Color.fromCssColorString(index===0?'#f4d35e':'#ffffff').withAlpha(.5),outlineColor:C.Color.fromCssColorString('#0b2922').withAlpha(.5),outlineWidth:4,eyeOffset:new C.Cartesian3(0,0,-20),heightReference:C.HeightReference.CLAMP_TO_GROUND,disableDepthTestDistance:Number.POSITIVE_INFINITY},label:{text:index===0?'TEE':index===axis.length-1?`${Math.round(direct)} m · GREEN`:`${Math.round(direct)} m`,font:'600 13px Inter, system-ui, sans-serif',fillColor:C.Color.fromCssColorString('#08251f'),showBackground:true,backgroundColor:C.Color.fromCssColorString(index===0?'#f4d35e':'#ffffff').withAlpha(.5),pixelOffset:new C.Cartesian2(0,-28),eyeOffset:new C.Cartesian3(0,0,-20),heightReference:C.HeightReference.CLAMP_TO_GROUND,disableDepthTestDistance:Number.POSITIVE_INFINITY}});entity.addProperty('axisHandle');entity.axisHandle={hole:state.view,index};axisEntities.push(entity)});viewer.scene.requestRender();
}
...
function selectView(id){
  ...
  $('map-title').textContent=state.view==='overview'?'Hela banan':`Hål ${state.view}`;clearEntities(routeEntities);clearEntities(distanceEntities);redrawGuides();drawAreasAndAxis();updateGuideUi();updateBoundaryUi();updateFlight();if(state.view==='overview'||!applyHoleView(state.view))reset();
