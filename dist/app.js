...
function drawAreasAndAxis(){
  ...
  for(const area of state.areas.filter(a=>String(a.hole)===state.view)){
    const [color,label]=areaVisualStyle(area),labelPlacement=areaLabelPosition(area),displayName=(area.name||`${label} (Hål ${area.hole})`).trim(),selected=area.id===selectedAreaId,rounded=area.type==='green',feathered=area.type==='tee'||area.type==='green',renderPoints=rounded?smoothClosedPolygon(area.points):area.points;
