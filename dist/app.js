...
function drawAreasAndAxis(){
  ...
  for(const area of state.areas.filter(a=>String(a.hole)===state.view)){
    const [color,label]=areaStyle[area.type]||areaStyle.other,labelPlacement=areaLabelPosition(area),displayName=(area.name||`${label} (Hål ${area.hole})`).trim(),selected=area.id===selectedAreaId;
    ...
    const areaLabel=viewer.entities.add({position:C.Cartesian3.fromDegrees(...labelPlacement.position),label:{text:displayName,font:'bold 13px sans-serif',fillColor:C.Color.fromCssColorString('#08251f'),showBackground:true,backgroundColor:C.Color.fromCssColorString(color).withAlpha(.92),backgroundPadding:new C.Cartesian2(8,5),pixelOffset:labelPlacement.pixelOffset,verticalOrigin:labelPlacement.verticalOrigin,heightReference:C.HeightReference.CLAMP_TO_GROUND,disableDepthTestDistance:Number.POSITIVE_INFINITY}});areaLabel.addProperty('areaId');areaLabel.areaId=area.id;areaEntities.push(areaLabel);
