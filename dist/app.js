...
function layoutMapLabels(){if(!viewer||!mapLabelLayout.length)return;const placed=[];for(const item of [...mapLabelLayout].sort((a,b)=>a.priority-b.priority)){const position=item.entity.position?.getValue?.(C.JulianDate.now())||item.entity.position,screen=position&&C.SceneTransforms.worldToWindowCoordinates(viewer.scene,position);if(!screen)continue;let chosen=0;for(const dx of [0,item.width*.62,-item.width*.62,item.width*1.24,-item.width*1.24,item.width*1.86,-item.width*1.86]){const box={left:screen.x+dx-item.width/2,right:screen.x+dx+item.width/2,top:screen.y+item.baseY-item.height,bottom:screen.y+item.baseY};if(!placed.some(other=>labelsOverlap(box,other))){chosen=dx;placed.push(box);break}}const offset=new C.Cartesian2(chosen,item.baseY);if(item.entity.billboard)item.entity.billboard.pixelOffset=offset;else if(item.entity.label)item.entity.label.pixelOffset=offset}viewer.scene.requestRender()}
